...
print("Registros después de eliminar nulos:", df_clean.shape[0])

print(df.dtypes)

# ==========================
# LIMPIEZA + CONVERSIÓN DE TIPOS
# ==========================

# Cargamos el dataset original (df) para transformarlo en df_clean
# df = pd.read_csv('DeltaDeDatos.csv') # Asegúrate de tener df cargado
df_clean = df.copy()

# 1) Normalizar nombres de columnas
# Esto elimina espacios, puntos y saltos de línea para evitar errores en las fórmulas
df_clean.columns = (
    df_clean.columns
    .astype(str)
    .str.strip()
...
int_columns = ['Posición', 'Año', 'Cant_Empleados']
...
# 5) Convertir categóricas (Reduce drásticamente el uso de RAM frente a 'object')
for col in categorical_columns:
    if col in df_clean.columns:
        df_clean[col] = df_clean[col].astype('category')

# 6) Gestión de Filas: Nulos y Duplicados
print(f"Registros iniciales: {df_clean.shape[0]}")

# Eliminar filas con nulos en columnas críticas (evita errores en cálculos financieros)
...
# 7) Verificación de memoria y tipos
df_clean.info()
...
# Crea una variable temporal con las 20 filas y guárdala
...
# 2. Creación de variables predictoras (Indicadores Financieros)
epsilon = 1e-7  # Evita división para cero
...
# Clasificación multiclase en 3 niveles basados en el Margen Neto
...
epsilon = 1e-7  # Constante para evitar división por cero

# Cálculo de los 6 Indicadores Predictores (Metodología)
...
# 2.2) Limpieza de valores extremos generados por el cálculo
...
# 2.3) Creación de la Variable Objetivo (Target)
# Clasificamos en 3 niveles de desempeño (Bajo, Medio, Alto) basados en el Margen Neto
# Usamos duplicates='drop' para manejar casos donde los límites de los cuartiles coincidan
...
# Verificación de que no queden nulos tras los cálculos
...
# =========================================================
# 4. FILTRADO POR AÑO Y PREPROCESAMIENTO EFICIENTE
# =========================================================

# 1) Filtrado por Año 2024
# Reducimos el dataset inmediatamente para acelerar el entrenamiento
df_modelo = df_clean[df_clean['Año'] == 2024].copy()

# Verificación de seguridad
if df_modelo.empty:
    print("❌ Error: No hay datos para el año 2024. Revisa el dataset o usa el año 2023.")
else:
    print(f"✅ Registros filtrados para el año 2024: {df_modelo.shape[0]}")

    # 2) Selección de variables (Features) según la metodología
    ...
    cat_features = ['Región', 'Sector']

    # Definir matriz X y vector objetivo y
    X = df_modelo[num_features + cat_features]
    y = df_modelo['Desempeno']

    # 3) Definición del Preprocesador (Pipeline de transformación)
    # StandardScaler para numéricos y OrdinalEncoder para categorías
    ...
    # 4) División Estratificada (Train: 70%, Val: 15%, Test: 15%)
    ...
    # Es vital para realizar inferencias con nuevos datos más adelante
    joblib.dump(preprocessor, 'preprocessor_2025.pkl')

    print("\n✅ Preprocesamiento eficiente completado.")
    print(f"--- Distribución final de datos ---")
    print(f"Entrenamiento (70%): {X_train_proc.shape[0]} registros")
    print(f"Validación    (15%): {X_val_proc.shape[0]} registros")
...
# 4.1 Configuración de Modelos con VERBOSE activado
...
models = {
    "Random Forest": RandomForestClassifier(
        ...
        verbose=1  # <--- Esto mostrará el progreso de construcción de los árboles
    ),
    "Gradient Boosting": GradientBoostingClassifier(
        ...
        max_depth=5,
        random_state=42,
        verbose=1  # <--- Mostrará el progreso de cada iteración de boosting
    )
}
...
for name, model in models.items():
    ...
    # El output aparecerá justo debajo de la celda mientras entrena
    ...
    print(f"\n✅ {name} finalizado. F1-Score obtenido: {f1:.4f}")
...
print("\n" + "*"*40)
print(f"🏆 MEJOR MODELO PARA 2024: {mejor_nombre}")
print("*"*40)

# 4.3 Informe Final de Negocio
print("\nReporte Metodológico de Clasificación:")
print(classification_report(y_val, mejor_modelo.predict(X_val_proc)))

# 4.4 Guardar para Implementación
...
# =========================================================
# 5. EVALUACIÓN DE MÉTRICAS Y EXPLICABILIDAD
# =========================================================
...
# 5.2 Visualización de la Matriz de Confusión (Normalizada)
...
# 5.3 Heatmap de Métricas (Precision, Recall, F1)
plt.subplot(1, 2, 2)
...
report_df = pd.DataFrame(report_dict).iloc[:-1, :3].T # Excluimos métricas globales para el heatmap por clase

sns.heatmap(report_df, annot=True, cmap='RdYlGn', fmt='.2f')
plt.title('Métricas Detalladas: Precision, Recall y F1', fontsize=12)
plt.show()
...
plt.title(f'¿Qué indicadores financieros influyen más en el Desempeño?', fontsize=13)
plt.xlabel('Poder Predictivo (Importancia)')
plt.grid(axis='x', linestyle='--', alpha=0.5)
plt.tight_layout()
plt.show()

# 5.5 Resumen de Evaluación Técnica (Criterio de Éxito)
print(f"\n" + "="*55)
print(f"📊 RESUMEN DE EVALUACIÓN FINAL - AÑO 2024")
print(f"="*55)
print(f"Mejor Modelo Seleccionado: {mejor_nombre}")
print(f"Métrica F1-Score (Weighted): {f1_final:.4f}")

# Evaluación contra tu meta metodológica
meta = 0.75
if f1_final >= meta:
    print(f"✅ ESTADO: ÉXITO TÉCNICO ALCANZADO (Superó la meta de {meta})")
else:
    print(f"⚠️ ESTADO: BAJO LA META (Meta {meta} no alcanzada)")

print("-" * 55)
# Extraer la clase con mejor desempeño del diccionario
mejor_clase = report_df['f1-score'].idxmax()
print(f"Indicador con mayor peso: {feat_importancia_df.iloc[-1]['Indicador']}")
print(f"Clase clasificada con mayor precisión: {mejor_clase}")
print("="*55)
...
# =========================================================
# 6. INFERENCIA: CLASIFICACIÓN DE NUEVAS EMPRESAS
# =========================================================
...
def clasificar_nueva_empresa(datos_dict, modelo_a_usar, preprocesador_a_usar):
    ...
    y devuelve la categoría de desempeño con su probabilidad.
    """
    # 1. Convertir el diccionario a DataFrame
    df_new = pd.DataFrame([datos_dict])

    # 2. Asegurar que las columnas estén en el orden correcto que espera el preprocesador
...
# Estos datos simulan la entrada de una empresa para el año 2025
empresa_test = {
    'Cant_Empleados': 120,
...
print("="*40)
print("📊 RESULTADO DE LA EVALUACIÓN 2024")
print("="*40)
print(f"Clasificación sugerida: {resultado.upper()}")
print(f"Nivel de Confianza:    {score:.2%}")
print("-"*40)

# Interpretación para el usuario/negocio
if resultado == 'Alto':
    print("Sugerencia: Empresa con sólidos indicadores. Candidata para inversión/crédito.")
elif resultado == 'Bajo':
    print("Sugerencia: Se recomienda auditoría o revisión de liquidez y carga tributaria.")
else:
    print("Sugerencia: Desempeño estable dentro del promedio del sector.")